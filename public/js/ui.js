$(function() {
	...
	const authmodalPanes = $authmodal.find('.auth-box');

	// Start on the right pane...Defaults to 'join'...Options 'signin' | 'join' | 'password'

	// MAKESHIFT WAY TO EXPOSE JQUERY AUTH LOGIC TO REACT
	window.signinModalTrigger = function signinModalTrigger(e) {
		e.preventDefault();

		const initial = $(this).data('initial') || 'join';
		const initialPane = $authmodal.find('.modal-pane-' + initial);
		...
		// only focus the first field on large devices where showing the keyboard isn't a jarring experience
		if ($(window).width() >= 768) {
	...
	// Handle attendence
	// ------------------------------
	// const $nextMeetup = $('#next-meetup');
	// if ($nextMeetup.length) {
	// 	if (!$('.meetup-toggle').length) return;
	// 	const meetup = $nextMeetup.data();
	// 	const $attending = $('.js-rsvp-attending'),
	// 		$decline = $('.js-rsvp-decline');
	// 	const toggleRSVP = function(attending) {

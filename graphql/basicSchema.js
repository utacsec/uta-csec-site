...
const meetupType = new GraphQLObjectType({
	...
	fields: () => ({
		...
		talks: {
			...
			resolve: (source, args) => Talk.model.find().where('meetup', source.id).exec(),
		},
		rsvps: {
			type: new GraphQLList(rsvpType),
			resolve: (source, args) => RSVP.model.find().where('meetup', source.id).exec(),
		},
	}),
});
...
const userType = new GraphQLObjectType({
	...
	fields: () => ({
		...
		talks: {
			type: new GraphQLList(talkType),
			resolve: (source, args) => Talk.model.find().where('who', source.id).exec(),
		},
		rsvps: {
			type: new GraphQLList(rsvpType),
			resolve: (source, args) => RSVP.model.find().where('who', source.id).exec(),
		},
	}),
});
...
const sponsorType = new GraphQLObjectType({
	...
	fields: () => ({
		...
		members: {
			type: new GraphQLList(userType),
			resolve: (source, args) => User.model.find().where('sponsor', source.id).exec(),
		},
	}),
});
...
const queryRootType = new GraphQLObjectType({
	...
	fields: {
		meetups: {
			...
			resolve: (_, args) => Meetup.model.find().exec(),
		},
		meetup: {
			type: meetupType,
			args: {
				id: { description: 'id of the meetup, can be "next" or "last"', type: new GraphQLNonNull(GraphQLID), },
			},
		...
		talk: {
			type: talkType,
			args: {
				id: { description: 'id of the talk', type: new GraphQLNonNull(GraphQLID), },
			},
		...
		sponsor: {
			type: sponsorType,
			args: {
				id: { description: 'id of the sponsor', type: new GraphQLNonNull(GraphQLID), },
			},
			resolve: (_, args) => Sponsor.model.findById(args.id).exec(),
		},
		users: {
			type: new GraphQLList(userType),
		...
		user: {
			type: userType,
			args: {
				id: { description: 'id of the user', type: new GraphQLNonNull(GraphQLID), },
			},
		...
		rsvp: {
			type: rsvpType,
			args: {
				id: { description: 'id of the RSVP', type: new GraphQLNonNull(GraphQLID), },
			},

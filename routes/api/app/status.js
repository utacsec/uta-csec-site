...
exports = module.exports = function (req, res) {
	...
	async.series([
		...
		function (next) {
			...
			keystone.list('Talk').model.find()
				...
				.exec(function (err, talks) {
					data.talks.last = talks && talks.length ? talks.map(function (i) { return i.toJSON(); }) : false;
		...
		const response = {
			...
			meetups: { last: false, next: false, },
			rsvp: { responded: false, attending: false, },
			user: false,
		};
		...
		if (data.meetups.next && moment().isBefore(data.meetups.next.endDate)) {
			...
			if (data.user) {
                response.rsvp.responded = !!data.rsvp;
                response.rsvp.attending = !!(data.rsvp && data.rsvp.attending);
		...
		if (data.user) {
			response.user = {
				...
				userId: data.user.id,
				name: { first: data.user.name.first, last: data.user.name.last, full: data.user.name.full, },

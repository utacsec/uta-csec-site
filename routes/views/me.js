...
exports = module.exports = function (req, res) {
	...
	locals.section = 'me';
	locals.page.title = 'Settings - UTA CSEC';
	
	view.query('nextMeetup', Meetup.model.findOne().where('state', 'active').sort('startDate'), 'talks[who]');
	view.query('rsvps.history',
        RSVP.model.find().where('who', req.user).where('attending', true).populate('meetup').sort('-createdAt')
    );
	
	view.on('post', { action: 'profile.details', }, function (next) {
		req.user.getUpdateHandler(req).process(req.body, {
			fields: 'name, email, notifications.meetups, notifications.posts, website, isPublic, bio, photo,'
